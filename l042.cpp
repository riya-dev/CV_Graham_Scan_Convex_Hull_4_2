// cd project4
// cd part2
// g++ -std=c++11 -o l042  -Wall l042.cpp
// ./l042
...
public:
    ...
    static bool sortFunctAngle(Point i, Point j) { return (j.getangle() > i.getangle()); }
    static std::vector<Point> getSortedAngle(std::vector<Point> p) {
        std::sort(p.begin(), p.end(), sortFunctAngle);
        return p;
    }
    void print();
...
Point::Point(double xval, double yval, double myAngle) {
    x = xval;
    y = yval;
    angle = myAngle;
}
...
void Point::setangle(double myAngle) {
    angle = myAngle;
}
...
void displayhull() {
    std::cout << "convexhull ";
    for (size_t i = 0; i < quickhull.size(); i++) {
        quickhull[i].print();
        std::cout << " ";
    }
    std::cout << "\n";
...
void FindHull(std::vector<Point>& v, Point& A, Point& B) {
    ...
    for (size_t i = 0; i < quickhull.size() - 1; i++) {
        //if (convexhull[i].getX() == A.getX() && convexhull[i].getY() == A.getY() && convexhull[i + 1].getX() == B.getX() && convexhull[i+1].getY() == B.getY())
        if (quickhull[i].same(A) && quickhull[i + 1].same(B)) {
            quickhull.insert(quickhull.begin() + i + 1, C);
            //displayhull();
        }
    }
...
void part1() {
    ...
    //B.print();
    quickhull.push_back(A);
    quickhull.push_back(B);
    quickhull.push_back(A);
    //displayhull();
    ...
    for (size_t i = 0; i < quickhull.size() - 1; i++) {
        //convexhull[i].print();
        //std::cout << "\n";
        chooselinefunction((int)(quickhull[i].getX() * size), (int)(quickhull[i + 1].getX() * size), (int)(quickhull[i].getY() * size), (int)(quickhull[i + 1].getY() * size), array);
    }

    for (int i = 0; i < size; ++i) {
        for (int j = 0; j < size; ++j) {
            if (array[i][j] == 0) // black
                ppm << array[i][j] << " " << array[i][j] << " " << array[i][j] << " ";
            else if (array[i][j] == 1) // white
                ppm << 255 << " " << 255 << " " << 255 << " ";
            else if (array[i][j] == 2) // red
                ppm << 255 << " " << 0 << " " << 0 << " ";
            else // blue
                ppm << 0 << " " << 0 << " " << 255 << ' ';
        }
...
void part2() {
    ...
    for (it = vec.begin(); it != vec.end(); it++) {
        ...
        circle(array, (int)(std::stod(s1) * size), (int)(std::stod(s2) * size), 3, 0);
    }

    // get greatest y coordinate to get the lowest point
    Point p = Point::getSortedY(v)[v.size()-1];
    //circle(array, (int)(p.getX() * size), (int)(p.getY() * size), 3, 2);
    grahamhull.push(p);
    //p.print();
    //std::cout << "\nlowest y coordinate\n";
    ...
    for (size_t i = 0; i < v.size(); i++) {
        //if (v[i].diff(p))
        double angle = abs(atan2(p.getY() - v[i].getY(), p.getX() - v[i].getX()));
        v[i].setangle(angle);
    }

    v = Point::getSortedAngle(v);

    /*for (Point temp : v) {
        temp.print();
        std::cout << temp.getangle();
        std::cout << "\n";
        //chooselinefunction((int)(temp.getX() * size), (int)(p.getX() * size), (int)(temp.getY() * size), (int)(p.getY() * size), array);
    }*/
    ...
    double turn;
    grahamhull.push(v[0]);
    for (size_t temp = 1; temp < v.size(); temp++) {
        current = v[temp];
        while (grahamhull.size() >= 2) {
            top = grahamhull.top();
            grahamhull.pop();
            prev = grahamhull.top();
            grahamhull.push(top);
            turn = turnvalue(prev, top, current);
            if (turn >= 0) { // left turn
                break;
            }
    ...
    // connect dots of quickhull
    while (grahamhull.size() > 1) {
        Point a = grahamhull.top();
        grahamhull.pop();
        Point b = grahamhull.top();
        chooselinefunction((int)(a.getX() * size), (int)(b.getX() * size), (int)(a.getY() * size), (int)(b.getY() * size), array);
    }

    /*std::vector <Point> hull;

    while (quickhull.size() > 0) {
        //quickhull.top().print();
        //std::cout << "\n";
        hull.push_back(quickhull.top());
        quickhull.pop();
    }
    for (size_t i = 0; i < hull.size() - 1; i++) {
        chooselinefunction((int)(hull[i].getX() * size), (int)(hull[i + 1].getX() * size), (int)(hull[i].getY() * size), (int)(hull[i + 1].getY() * size), array);
    }*/
